...
SEARCH_QUERY_GENERATOR = ChatPromptTemplate.from_messages(
    [
        (
            ...
            " queries provided by you. Each query shoudl not contain more than 3-6 words. Only return a max of 2 search queries (contextually different from one another so they don't return the same search results) Reply in valid JSON with search_queries as the key",
        ),
        (
            "user",
